...
//
// Creates a Stripe Checkout Session using the raw Stripe REST API (no npm
// dependency needed, works with Netlify's zero-config function bundling).
//
// Requires an environment variable set in Netlify:
//   STRIPE_SECRET_KEY = sk_live_xxx  (or sk_test_xxx while testing)
//
// Apple Pay / Google Pay show up automatically on Stripe's hosted Checkout
// page as long as they're enabled in Stripe Dashboard → Settings → Payment
// methods. No domain verification is needed for hosted Checkout (only
// needed if you embed Apple Pay directly on your own domain).
...
exports.handler = async (event) => {
  ...
  const { items, deliveryName, deliveryPrice, summary } = body;
  if (!Array.isArray(items) || items.length === 0) {
    return { statusCode: 400, body: JSON.stringify({ error: 'السلة فاضية' }) };
  }

  const siteUrl = process.env.URL || `https://${event.headers.host}`;
  const successUrl = summary
    ? `${siteUrl}/success.html?summary=${encodeURIComponent(summary)}`
